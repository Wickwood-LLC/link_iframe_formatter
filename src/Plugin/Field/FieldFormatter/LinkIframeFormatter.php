...
class LinkIframeFormatter extends LinkFormatter {
  ...
  public function settingsForm(array $form, FormStateInterface $form_state) {
    ...
    $elements['min_width'] = array(
      '#title' => $this->t('Minimum Width'),
      '#type' => 'textfield',
      '#description' => $this->t('Optionally include the unit, e.g. px, vh. Defaults to px.'),
      '#default_value' => $this->getSetting('min_width'),
      '#size' => 20,
      '#states' => array(
        'visible' => array(
    ...
    $elements['min_height'] = array(
      '#title' => $this->t('Minimum Height'),
      '#type' => 'textfield',
      '#description' => $this->t('Optionally include the unit, e.g. px, vh. Defaults to px.'),
      '#default_value' => $this->getSetting('min_height'),
      '#size' => 20,
      '#states' => array(
        'visible' => array(
          ':input[name*="responsive"]' => array('checked' => TRUE),
  ...
  public function settingsSummary() {
    ...
    if ($this->getSetting('responsive')) {
      $dimensions = $this->t('Responsive with min-width: @min_width, max-width: @max_width, min-height: @min_height, max-height: @max_height', array(
        '@min_width' => $this->getSetting('min_width'),
        '@max_width' => $this->getSetting('max_width'),
        '@min_height' => $this->getSetting('min_height'),
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    foreach ($items as $delta => $item) {
      ...
      if ($settings['responsive']) {
        ...
        if (is_numeric(trim($min_width))) {
          $min_width .= 'px';
        }
        $max_width = $settings['max_width'];
        if (is_numeric(trim($max_width))) {
          $max_width .= 'px';
        }
        $min_height = $settings['min_height'];
        if (is_numeric(trim($min_height))) {
          $min_height .= 'px';
        }
        $max_height = $settings['max_height'];
        if (is_numeric(trim($max_height))) {
          $max_height .= 'px';
        }
      }
      else {
